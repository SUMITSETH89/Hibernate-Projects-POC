...
public class CompanyMgtDAOImpl implements ICompanyMgtDAO {
	...
	public void saveDataUsingChild() {
		...
		try(Session sess = HibernateUtil.getSession()){
			...
			emp1.setEname("Anand");
			emp1.setDesignation("Scientist-B");
			emp1.setSalary(77000.00);
			
			
			Employee emp2 = new Employee();
			emp2.setEname("Harshit");
			emp2.setDesignation("Scientist-C");
			emp2.setSalary(60000.00);
			
			
			//Add Childs to Parent
			dept.setEmployees(Set.of(emp,emp1,emp2));
			
			//Add Parent to Child
			emp.setDept(dept); emp1.setDept(dept);emp2.setDept(dept);
			
			//Save Though Child
			Long id1 = (Long)sess.save(emp);
			Long id2 = (Long)sess.save(emp1);
			Long id3 = (Long)sess.save(emp2);
			
			//commit Transaction
			tx.commit();
			
			System.out.println("Childs With ID's:: "+id1+","+id2+","+id3+" Persisted Successfully");
	...
	public void fetchByChildWithProxy() {
		...
		try(Session sess = HibernateUtil.getSession()){
			//Query object represents HQL Query
			Query<Employee> query = sess.createQuery("FROM Employee ORDER BY empId",Employee.class);
			//Execute Query
	...
	public void fetchByChildWithNoProxy() {
		...
		try(Session sess = HibernateUtil.getSession()){
		    ...
		    empList.forEach(emp->{
		    	System.out.println(emp);
		    	//IDepartmentProxy dept = emp.getDept();//Update
